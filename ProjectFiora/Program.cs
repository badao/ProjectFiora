...
namespace FioraProject
{
    public static class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //evade
            FioraProject.Evade.Evade.Evading += Evading;
            Game.PrintChat("Welcome to FioraWorld");
        }
        private static int Manaharass { get { return Menu.Item("Mana Harass").GetValue<Slider>().Value; } }
        private static int ManaJclear { get { return Menu.Item("minimum Mana JC").GetValue<Slider>().Value; } }
        private static int ManaLclear { get { return Menu.Item("minimum Mana LC").GetValue<Slider>().Value; } }
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            if (Player.IsDead)
                return;
            if (DrawQ)
                DrawCircle(Player.Position, 400, Color.Green);
            if (DrawW)
            {
                DrawCircle(Player.Position, W.Range, Color.Green);
            }
            if (DrawOptionalRange && TargetingMode == TargetMode.Optional)
            {
                DrawCircle(Player.Position, OptionalRange, Color.DeepPink);
            }
            if (DrawSelectedRange && TargetingMode == TargetMode.Selected)
            {
                DrawCircle(Player.Position, SelectedRange, Color.DeepPink);
            }
            if (DrawPriorityRange && TargetingMode == TargetMode.Priority)
            {
                DrawCircle(Player.Position, PriorityRange, Color.DeepPink);
            }
            if (DrawTarget && TargetingMode != TargetMode.Normal)
            {
                var hero = GetTarget();
                if (hero != null)
                    DrawHinh1(hero.Position, 75, Color.Yellow, 5);
            }
            ...
            if (usewalljump && activewalljump)
            {
                ...
                for (int i = 20; i <= 300; i= i + 20)
                {
                    if (Utils.GameTimeTickCount - movetick < (70 + Math.Min(60, Game.Ping)))
                        break;
                    if(Player.Position.To2D().Extend(Game.CursorPos.To2D(),i).IsWall())
                    {
                    ...
                    Player.IssueOrder(GameObjectOrder.MoveTo, Player.Position.To2D().Extend(Game.CursorPos.To2D(), 200).To3D());
                }
        ...
        private static void OrbwalkToPassive()
        {
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.OrbwalkPassive)
            {
                ...
                if (target.IsValidTarget(400) && !target.IsZombie)
                {
        ...
        private static bool CastQGapCloseHarass { get { return Menu.Item("Use Q Harass Gap").GetValue<bool>(); } }
        private static bool CastQPrePassiveHarass { get { return Menu.Item("Use Q Harass Pre Pass").GetValue<bool>(); } }
        private static bool CastQPassiveHarasss { get { return Menu.Item("Use Q Harass Pass").GetValue<bool>(); } }
        ...
        private static bool CastQGapCloseCombo { get { return Menu.Item("Use Q Combo Gap").GetValue<bool>(); } }
        private static bool CastQPrePassiveCombo { get { return Menu.Item("Use Q Combo Pre Pass").GetValue<bool>(); } }
        private static bool CastQPassiveCombo { get { return Menu.Item("Use Q Combo Pass").GetValue<bool>(); } }
        private static bool CastQMinionGapCloseCombo { get { return Menu.Item("Use Q Combo Gap Minion").GetValue<bool>(); } }
        private static int ValueQMinionGapCloseCombo { get { return Menu.Item("Use Q Combo Gap Minion Value").GetValue<Slider>().Value; } }
        ...
        private static void Combo()
        {
            ...
            if (Q.IsReady() && Qcombo)
            {
                ...
                //        {
                //            var status = hero.GetPassiveStatus(0);
                //            if (status.HasPassive && status.PassiveType == PassiveType.UltiPassive && !(Orbwalking.InAutoAttackRange(hero)
                //                && status.PassivePredictedPositions.Any(x => Player.Position.To2D()
                //                    .InTheCone(status.TargetPredictedPosition, x, 90))) && castQtoUltPassive(hero, getQPassivedelay(hero)))
                //                goto Wcast;
                //        }
                //    }
                //    else
                ...
                //        {
                //            var status = hero.GetPassiveStatus(0);
                //            if (status.HasPassive && status.PassiveType == PassiveType.NormalPassive && !(Orbwalking.InAutoAttackRange(hero)
                //                && status.PassivePredictedPositions.Any(x => Player.Position.To2D()
                //                    .InTheCone(status.TargetPredictedPosition, x, 90))) && castQtoPassive(hero, getQPassivedelay(hero)))
                //                goto Wcast;
                //        }
                //    }
                //    else
                ...
                //        {
                //            var status = hero.GetPassiveStatus(0);
                //            if (status.HasPassive && status.PassiveType == PassiveType.PrePassive && !(Orbwalking.InAutoAttackRange(hero)
                //                && status.PassivePredictedPositions.Any(x => Player.Position.To2D()
                //                    .InTheCone(status.TargetPredictedPosition, x, 90))) && castQtoPrePassive(hero, getQPassivedelay(hero)))
                //                goto Wcast;
                //        }
                //    }
                //    else
                ...
                //if (CastQGapCloseCombo)
                //{
                //    if (!CastQPassiveCombo && !CastQPrePassiveCombo)
                //    {
                //        if (TargetingMode == TargetMode.Normal)
                //        {
                //            foreach (var hero in HeroManager.Enemies.Where(x => x.IsValidTarget() && !x.IsZombie)
                ...
                //            {
                //                var status = hero.GetPassiveStatus(0);
                //                if (status.HasPassive && !(Orbwalking.InAutoAttackRange(hero)
                //                    && status.PassivePredictedPositions.Any(x => Player.Position.To2D()
                //                        .InTheCone(status.TargetPredictedPosition, x, 90))))
                //                    if (castQtoGapClose(hero, getQGapClosedelay(hero)))
                //                        goto Wcast;
        ...
        #region CastWHelper
        //private static List<Skillshot> SkillShots = new List<Skillshot>();
        //private static bool IsActiveBlockSkillShot(Obj_AI_Hero Caster,SpellSlot Slot )
        //{
        //    return Menu.Item(Caster.ChampionName.ToString() + Slot.ToString()).GetValue<bool>();
        //}
        //private static bool EnableWBlocking;
        //private static bool WBlockSkillShot;
        //private static bool WBlockTargeted;
        //private static void WBlockSkillShotOnUpdate()
        //{
        //    if (!EnableWBlocking || !WBlockSkillShot)
        //        return;
        //    SkillShots.RemoveAll(x => x.Missile == null);
        //    if (W.IsReady())
        //        foreach (var x in SkillShots)
        //        {
        //            if (x.SpellData.Type == SkillShotType.SkillshotLine)
        //                if (Player.Position.To2D().Distance(x.Missile.Position.To2D(),x.Missile.EndPosition.To2D(),true)
        //                    <= Player.BoundingRadius + x.SpellData.Radius
        //        }
        //}
        ...
        //        return;
        //    }
        //    if (!IsActiveBlockSkillShot(Caster,spellData.Slot))
        //    {
        //        return;
        //    }
        //    var skillshot = new Skillshot(missile, spellData);
        //    if (Collision.GetCollisionPoint(skillshot).Distance(skillshot.Missile.StartPosition.To2D()) + 100
        //        < Player.Position.To2D().Distance(skillshot.Missile.StartPosition.To2D()))
        //        return;
        //    SkillShots.Add(skillshot);
        //}
        //private static readonly List<Wblock> ChampionsBlock = new List<Wblock>()
        //{
        //    new Wblock
        //        (
        //        )
        //};
        private class Wblock
        {
            public string ChampionName;
            public List<WblockMethod> WblockMethods = new List<WblockMethod>();
            public Wblock(string championName, List<WblockMethod> wblockMethods)
            {
                ChampionName = championName;
                WblockMethods = wblockMethods;
            }
        }
        private class WblockMethod
        {
            public SpellSlot Slot;
            public int ActionDelay;
            public Vector2? Position;
            public WblockMethod(SpellSlot slot, int actionDelay, Vector2? position = null)
            {
                Slot = slot;
